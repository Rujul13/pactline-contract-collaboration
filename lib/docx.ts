...
export type ExportClause = { number: string; title: string; text: string };
export type DocumentBlock = { id: string; text: string; kind: "title" | "heading" | "body" };

const xml = (value: string) => value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");

export function createDocx(title: string, version: number, clauses: ExportClause[]) {
  const paragraphs = clauses.map((clause) => `
    <w:p><w:pPr><w:pStyle w:val="Heading2"/></w:pPr><w:bookmarkStart w:id="${clause.number}" w:name="clause_${clause.number}"/><w:r><w:t>${xml(`${clause.number}. ${clause.title}`)}</w:t></w:r><w:bookmarkEnd w:id="${clause.number}"/></w:p>
    <w:p><w:r><w:t xml:space="preserve">${xml(clause.text)}</w:t></w:r></w:p>`).join("");
  const files: Record<string, Uint8Array> = {
    ...
    "word/document.xml": strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:pPr><w:pStyle w:val="Title"/></w:pPr><w:r><w:t>${xml(title)}</w:t></w:r></w:p><w:p><w:r><w:t>${xml(`Contract version ${version}`)}</w:t></w:r></w:p>${paragraphs}<w:sectPr><w:pgSz w:w="12240" w:h="15840"/><w:pgMar w:top="1440" w:right="1440" w:bottom="1440" w:left="1440"/></w:sectPr></w:body></w:document>`),
