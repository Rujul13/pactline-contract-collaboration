...
- **6. Monitoring coverage**: `withMonitoring(handler, route)` wrapper applied to the four operational route files (GET `/api/contracts/[contractId]/relationships`, GET `/api/owner/monitoring/errors`, POST `/api/owner/monitoring/errors/[errorId]/resolve`, and GET `/api/owner/release-readiness`); per-request `request_id` generation, echoed in error responses; optional `MONITORING_ALERT_WEBHOOK_URL` for critical severity, no-op if unset.
