...
export default function CustomerReviewPage({ params }: { params: Promise<{ contractId: string }> }) {
  ...
  async function submitChanges() {
    ...
    setDrafts({}); setEditingId(null); setMessageType("success"); setMessage(`${edits.length} proposed ${edits.length === 1 ? "change" : "changes"} sent to the vendor workspace.`); await loadWorkspace();
  }
  ...
  async function agree() {
    ...
    setMessageType("success"); setMessage(result.locked ? "Both parties agreed. The final document is locked." : result.approvalPending ? "Your agreement is recorded. Required internal approval is still pending." : "Your agreement is recorded for this version. Waiting for the vendor."); await loadWorkspace();
  }
  ...
  return <main className="review-portal workspace-open">
    ...
    <article className="word-page portal-document"><div className="word-page-meta"><span>{locked ? "Final agreed document" : "Secure shared document"}</span><span>{workspace.blocks.length} paragraphs · {workspace.comments.filter((item) => item.status === "open").length} open comments</span></div><div className="document-flow">
      {workspace.blocks.map((block) => {
        ...
        return <section className={`doc-paragraph ${block.kind} ${draft ? "staged" : ""} ${ownerCounter ? "counter-returned" : ""}`} key={block.id}>
          ...
          {ownerCounter && !draft && <div className="client-counter-card"><strong>Vendor counterproposal</strong><p>{ownerCounter.counter_text}</p>{ownerCounter.resolution_reason && <small>Reason: {ownerCounter.resolution_reason}</small>}<button disabled={locked || clientAgreed} onClick={() => { setDrafts((current) => ({ ...current, [block.id]: ownerCounter.counter_text! })); setEditingId(block.id); }}>Continue negotiation with this text</button></div>}
    ...
    <footer className="client-submit-bar"><div><strong>{locked ? "Final document ready" : Object.keys(drafts).length ? `${Object.keys(drafts).length} changes ready to submit` : pending.length ? `${pending.length} ${pending.length === 1 ? "change" : "changes"} awaiting the vendor` : counters.length ? `${counters.length} vendor counterproposal${counters.length === 1 ? "" : "s"} to review` : clientAgreed ? "Your agreement is recorded" : "No unresolved proposals"}</strong><small>{locked ? "Download the exact version agreed by both parties." : "Agreements apply only to the current document version."}</small></div>{locked ? <button onClick={downloadFinal}>Download final DOCX</button> : Object.keys(drafts).length ? <button disabled={busy} onClick={() => void submitChanges()}>Submit proposed changes</button> : <button disabled={busy || pending.length > 0 || counters.length > 0 || clientAgreed} onClick={() => void agree()}>{clientAgreed ? "Agreement recorded" : counters.length ? "Review vendor counter" : "Agree to this version"}</button>}</footer>
  </main>;
}
