...
export async function POST(request: Request) {
  ...
  const statements = [
    env.DB.prepare("INSERT INTO contracts (id,title,initiator_id,approver_id,status,template_id,current_version,owner_organization_id,counterparty_organization_id,origin,created_at,updated_at) VALUES (?,?,?,?,'draft',?,1,?,?,'customer_template',?,?)").bind(contractId, title, owner.id, owner.id, templateId, organizationId, supplierOrganizationId, now, now),
    env.DB.prepare("INSERT INTO parties (id,contract_id,role,name,company,email,created_at,updated_at) VALUES (?,?,'initiator','Contract Owner',(SELECT name FROM organizations WHERE id=?),?,?,?)").bind(ownerPartyId, contractId, organizationId, auth.user.email, now, now),
    env.DB.prepare("INSERT INTO parties (id,contract_id,role,name,company,email,created_at,updated_at) VALUES (?,?,'counterparty',?,?,?, ?,?)").bind(supplierPartyId, contractId, body.reviewerName?.trim().slice(0, 120) || portalAccount?.display_name || "Supplier Reviewer", supplier.name, body.reviewerEmail?.trim().slice(0, 254) || portalAccount?.email || "supplier@example.test", now, now),
