...
export async function POST(request: Request) {
  ...
  const clientCompany = String(form.get("clientCompany") ?? "Client Company").trim().slice(0, 120) || "Client Company";
  const reviewerName = String(form.get("reviewerName") ?? "Client Reviewer").trim().slice(0, 120) || "Client Reviewer";
  ...
  try {
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO audit_log_entries (id, contract_id, actor_id, actor_display, action, target_type, target_id, version_number, request_id, metadata, created_at) VALUES (?, ?, ?, 'Contract Owner', 'contract.created', 'contract', ?, 1, ?, json(?), ?)").bind(crypto.randomUUID(), contractId, owner.id, contractId, crypto.randomUUID(), JSON.stringify({ filename: safeName, paragraphs: rows.length, scanStatus: "pending" }), now),
    ]);
  } catch (error) {
