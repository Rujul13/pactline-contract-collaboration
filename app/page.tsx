...
export default function Home() {
  ...
  async function resolveProposal(proposal: Proposal, action: "accept" | "reject" | "counter") {
    if (!contract) return; setBusy(true);
    const response = await fetch(`/api/contracts/${contract.id}/paragraph-proposals/${proposal.id}/resolve`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, counterText: action === "counter" ? counterDraft : undefined }) });
    const result = await response.json() as { error?: string };
    setBusy(false); if (!response.ok) { announce(result.error ?? "Unable to resolve the proposal"); return; }
    setCounteringId(null); setCounterDraft(""); setSelectedProposalId(null);
