...
export default function Home() {
  ...
  return <main className="app-shell document-workspace">
    ...
    <section className="workspace">
      ...
      <div className="content-grid"><section className="document-panel"><div className="tabs"><button className={tab === "document" ? "active" : ""} onClick={() => setTab("document")}>Document</button><button className={tab === "history" ? "active" : ""} onClick={() => setTab("history")}>Version history <span>{workspace?.versions.length ?? 0}</span></button><button className={tab === "activity" ? "active" : ""} onClick={() => setTab("activity")}>Audit activity</button><button className={`owner-lock-button ${clientAgreed && !ownerAgreed ? "ready" : ""}`} disabled={busy || locked || ownerAgreed || pending.length > 0} title={pending.length ? "Resolve every pending proposal first" : clientAgreed ? "The client approved this version; your approval will lock it" : "Approve this version and wait for the client"} onClick={() => void agreeAsOwner()}>{locked ? "🔒 Locked" : ownerAgreed ? "✓ Owner approved" : clientAgreed ? "🔒 Lock version" : "✓ Approve version"}</button><div className="version-select">Version {contract?.current_version}</div></div>
