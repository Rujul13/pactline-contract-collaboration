...
export default function Home() {
  ...
  const aiApplyBlockedReason = locked ? "The final contract is locked." : pending.length ? "Resolve pending customer proposals before applying an AI draft." : "";
  ...
  async function createContract() {
    ...
    const form = new FormData(); form.append("title", newTitle.trim()); form.append("document", newFile); form.append("clientCompany", "Customer Company"); form.append("reviewerName", "Customer Reviewer"); form.append("reviewerEmail", "customer@example.test");
  ...
  async function agreeAsOwner() {
    ...
    const confirmation = clientAgreed
      ...
      : `Approve version ${contract.current_version}? It will lock automatically when the customer approves this same version.`;
  ...
  if (error && !workspace) return <main className="review-portal"><section className="portal-card"><span className="brand-mark">P</span><h1>Pactline</h1><p>{error}</p>{signIn && <a className="primary-link" href={signIn}>Sign in to vendor workspace</a>}<button onClick={() => void loadWorkspace()}>Try again</button></section></main>;
  ...
  return <main className="app-shell document-workspace">
    ...
    <section className="workspace">
      ...
      <aside className="review-rail"><div className="rail-tabs"><button className={rail === "proposals" ? "active" : ""} onClick={() => setRail("proposals")}>Proposed edits <span>{pending.length}</span></button><button className={rail === "details" ? "active" : ""} onClick={() => setRail("details")}>Details</button></div>{rail === "proposals" ? <><div className="rail-heading"><div><h2>Customer changes</h2><p>Changed words are highlighted for quick review</p></div></div><div className="proposal-scroll">{!workspace?.proposals.length && <div className="all-clear"><span>✓</span><h3>No proposed edits yet</h3><p>Share customer access to begin a review round.</p></div>}{workspace?.proposals.map((proposal) => <article className={`proposal-card ${proposal.status !== "pending" ? "resolved" : ""} ${selectedProposalId === proposal.id ? "selected" : ""}`} key={proposal.id}><div className="proposal-meta"><span className="mini-avatar">CU</span><div><strong>{proposal.proposed_by_name}</strong><small>{proposal.username} · {new Date(proposal.created_at).toLocaleString()}</small></div><span className={proposal.status === "pending" ? "pending-dot" : `status-text ${proposal.status}`}>{proposal.status}</span></div><button className="proposal-jump" onClick={() => focusProposal(proposal)}><span>Paragraph {paragraphNumber.get(proposal.block_id)} edited</span><b>View in document →</b></button><div className="diff"><div className="removed"><span>−</span><p><DiffText original={proposal.original_text} proposed={proposal.proposed_text} side="original"/></p></div><div className="added"><span>+</span><p><DiffText original={proposal.original_text} proposed={proposal.proposed_text} side="proposed"/></p></div>{proposal.counter_text && <div className="countered-version"><span>↔</span><p><b>Vendor counter:</b> {proposal.counter_text}</p></div>}</div>{proposal.status === "pending" && <div className="decision-row"><button className="accept" disabled={busy} onClick={() => void resolveProposal(proposal, "accept")}>✓ Accept</button><button className="counter" disabled={busy} onClick={() => { focusProposal(proposal); setCounteringId(proposal.id); setCounterDraft(proposal.proposed_text); }}>Counter</button><button className="reject" disabled={busy} onClick={() => void resolveProposal(proposal, "reject")}>Reject</button></div>}</article>)}</div></> : <div className="details-panel"><h2>Document details</h2><dl><dt>Vendor</dt><dd>{ownerParty?.name}<br/><small>{ownerParty?.company}</small></dd><dt>Customer reviewer</dt><dd>{clientParty?.name}<br/><small>{clientParty?.email}</small></dd><dt>Source</dt><dd>{workspace?.documents[0]?.filename}<br/><small>Private object storage · Unscanned MVP file</small></dd><dt>Version</dt><dd>Version {contract?.current_version}<br/><small>{workspace?.versions.length} immutable snapshots</small></dd><dt>Vendor agreement</dt><dd>{ownerAgreed ? "Recorded" : "Waiting"}</dd><dt>Customer agreement</dt><dd>{clientAgreed ? "Recorded" : "Waiting"}</dd></dl><div className="agreement-panel"><strong>{locked ? "Final agreement complete" : clientAgreed ? "Customer approved this version" : "Ready to agree?"}</strong><p>{locked ? "This version is locked. Both parties can download the same final Word document." : pending.length ? "Resolve every pending proposal before agreeing." : clientAgreed ? "Lock this exact version by adding the vendor’s approval." : "Your approval applies only to the current version and resets if the document changes."}</p><button className="agreement-button" disabled={busy || locked || ownerAgreed || pending.length > 0} onClick={() => void agreeAsOwner()}>{ownerAgreed ? "Vendor approval recorded" : clientAgreed ? "Lock this version" : "Approve this version"}</button>{locked && <button className="secondary-download" onClick={() => contract && download(`/api/contracts/${contract.id}/download`)}>Download final DOCX</button>}</div>{contract?.id === DEMO_CONTRACT_ID && <button className="reset-demo-button" disabled={busy} onClick={() => void resetDemo()}>Reset generic demo</button>}</div>}</aside></div>
    </section>

    {shareOpen && credentials && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setShareOpen(false); }}><section className="share-modal" role="dialog" aria-modal="true" aria-labelledby="share-title"><button className="modal-close" aria-label="Close sharing dialog" onClick={() => setShareOpen(false)}>×</button><div className="modal-kicker">🔒 Named, password-protected review</div><h2 id="share-title">Share with Customer Reviewer</h2><p className="modal-lead">The customer can propose paragraph edits and agree to the final version. They cannot overwrite the vendor document.</p><div className="access-identity"><span className="client-avatar">CU</span><div><strong>{clientParty?.name}</strong><small>{clientParty?.email} · {clientParty?.company}</small></div><span className="access-role">Customer reviewer</span></div><div className="credential-grid"><label><span>Secure contract link</span><div><code>{credentials.link}</code><button onClick={() => void copy(credentials.link, "Link")}>Copy</button></div></label><label><span>Username</span><div><code>{credentials.username}</code><button onClick={() => void copy(credentials.username, "Username")}>Copy</button></div></label><label><span>Temporary password</span><div><code>{passwordVisible ? credentials.password : "••••••••••••••••"}</code><button onClick={() => setPasswordVisible((value) => !value)}>{passwordVisible ? "Hide" : "Show"}</button></div></label></div><div className="modal-actions"><a className="preview-login" href={credentials.link} target="_blank">Open customer login</a><button className="copy-package" onClick={() => void copy(`Contract: ${credentials.link}\nUsername: ${credentials.username}\nPassword: ${credentials.password}`, "Access package")}>Copy access package</button></div></section></div>}
    ...
    {aiOpen && <aside className="ai-drawer" role="dialog" aria-modal="false" aria-labelledby="ai-title">
      <header className="ai-drawer-header"><div><span className="ai-spark">✦</span><div><h2 id="ai-title">Contract assistant</h2><p>Vendor workspace · Groq-powered</p></div></div><button aria-label="Close AI assistant" onClick={() => setAiOpen(false)}>×</button></header>
