  ...
  return <main className="workflow-shell">
    ...
    <section className="workflow-grid">
      <form className="workflow-card lifecycle-card" onSubmit={(event) => { event.preventDefault(); if (isPredecessor) return; void mutate("lifecycle", { ...form, reviewDeadlineAt: instantOrUndefined(form.reviewDeadlineAt), noticePeriodDays: Number(form.noticePeriodDays), contractValue: form.contractValue }, "Lifecycle details saved and reminders rebuilt."); }}>
        ...
        <div className="workflow-form-row"><label>Risk level<select disabled={isPredecessor} value={form.riskLevel} onChange={(event) => setForm((current) => ({ ...current, riskLevel: event.target.value }))}>{["low","medium","high","critical"].map((item) => <option value={item} key={item}>{title(item)}</option>)}</select></label><label>Vendor owner<input value={contract.responsible_owner_name ?? "Vendor Owner"} disabled/></label></div>
